...
TEST(CUDA, BlackScholesCallCpp) {
    ...
    kcu::geometric_brownian_motion<kcu::run_type::cpp> sde(
        S_0, [&r, &sigma](double X_t, double t) {
            return std::make_pair(r * X_t, sigma * X_t);
        });

    kcu::euler_maruyama<kcu::run_type::cpp> simulater(100);

    const auto& payoff = [K](double S) { return std::max(S - K, 0.0); };
...
TEST(CUDA, BlackScholesPutCpp) {
    ...
    const auto& payoff = [K](double S) { return std::max(K - S, 0.0); };
