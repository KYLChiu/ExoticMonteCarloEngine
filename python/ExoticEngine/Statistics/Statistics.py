...
class GetStatistics:
    def __init__(self, termination_condition: TerminationCondition):
        ...
        self.__prev_running_sum: float = 0.0
    ...
    def get_std_dev(self) -> float:
        if self._paths_done < 2:
            raise Exception(
                f"Path count is less than 2, path count = {self._paths_done}"
            )
    ...
    def terminate(self, min_path: int = 5, max_path: int = 10000) -> bool:
        ...
        if self._paths_done <= min_path:
            ...
            print(
                f"WARNING! Maximum number of paths reached: path_count={self.get_path_count()}, max_path={max_path}"
            )
            return True
            ...
            return (
                abs(
                    self.get_mean()
                    - self.__prev_running_sum / (self.get_path_count() - 1)
                )
                <= criteria
            )
            ...
            if self._paths_done > criteria:
                print(
                    f"WARNING! path_count ({self.get_path_count()}) > termination criteria {criteria})"
                )
                return True
